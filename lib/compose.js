...
/**
 * Create a new instance of the Compose for the given app and list of generators.
 *
 * ```js
 * var compose = new Compose(app, ['a', 'b', 'c']);
 * ```
 * @param {Object} `instance` The current "app" instance (generator) to extend.
 * @param {Array} `generators` Array of generator names to iterate over.
 */

function Compose(instance, generators) {
...
Compose.prototype.pipeline = function() {
  if (typeof this.app.pipeline !== 'function') {
...
Compose.prototype.iterator = function(names, iteratorFn) {
  ...
  while (++idx < len) {
    ...
    try {
      ...
      if (this.app.hasListeners('error')) {
