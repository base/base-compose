...
/**
 * Create a new instance of the Compose for the given source and destination applications and list of generators.
 *
 * ```js
 * var compose = new Compose(src, dest, ['a', 'b', 'c']);
 * ```
 * @param {Object} `src` The "app" instance (generator) containing generators.
 * @param {Object} `dest` The current "app" instance (generator) to extend.
 * @param {Array} `generators` Array of generators or generator names from the `src` generator to iterate over.
 */

function Compose(src, dest, generators) {
...
Compose.prototype.data = function(prop) {
  if (typeof this.dest.data !== 'function') {
...
Compose.prototype.engines = function() {
  if (typeof this.dest.engine !== 'function') {
...
Compose.prototype.helpers = function() {
  if (typeof this.dest.helper !== 'function') {
...
Compose.prototype.questions = function() {
  if (typeof this.dest.questions === 'undefined') {
...
Compose.prototype.tasks = function(tasks) {
  if (typeof this.dest.task !== 'function') {
...
Compose.prototype.views = function(names, filter) {
  if (typeof this.dest.create !== 'function') {
  ...
  while (++idx < len) {
    ...
    try {
      if (typeof name === 'string') {
        generator = this.src.getGenerator(name);
      ...
      iteratorFn(generator, this.dest);
    } catch (err) {
      if (this.dest.hasListeners('error')) {
        this.dest.emit('error', err);
      } else {
        throw err;
      }
    }
